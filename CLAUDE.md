...
- Does not run at runtime (build-time packaging only)
- Does not modify Claude Code behavior directly (produces artifacts Claude loads)
- Does not auto-update installed plugins (manual reinstall required)
- Does not execute skills (Claude Code interprets the markdown)
- Does not validate skill effectiveness (validates structure, not behavior)

## Failure Modes

- **Validation failure**: `make validate` exits non-zero with specific validator output
- **Missing manifest resource**: Build fails listing missing file paths
- **Cross-reference broken**: `validate-xrefs.js` lists orphaned references
- **Frontmatter parse error**: Validator shows YAML parse error with line number
- **Build output conflict**: `make clean && make build` resolves stale artifacts

## State Locations

| What | Path |
|------|------|
| Source assets | `resources/` (skills, agents, commands, hooks, rules) |
| Manifests | `manifests/claude/*.json`, `manifests/codex/*.yaml` |
| Built output | `dist/` (gitignored, regenerated by build) |
| Installed plugins | `~/.claude/plugins/` (via `./install.sh`) |
