...
```
lamella/
  plugin-manifests/*.json   # Source of truth -- declares which resources belong to each plugin
  ...
  scripts/ci/                         # 8 validation scripts
  scripts/plugins/                    # build-plugin.sh, build-marketplace.sh, install-plugin.sh
  dist/                               # Built output (gitignored)
  docs/                               # Reference documentation
```

Build flattens category subdirectories:
`skills/core/brainstorming/` -> `dist/plugins/core/skills/brainstorming/`
...
- **Always applies?** Short standard or checklist? -> `rules/<lang>/`
- **Multi-step sequence with agent chain?** -> `workflows/`
- **Deep domain knowledge or specialized techniques?** -> `skills/<category>/`
...
- [ ] Added to at least one plugin manifest in `plugin-manifests/`
