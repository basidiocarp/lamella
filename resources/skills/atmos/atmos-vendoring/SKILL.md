...
description: "Component vendoring: vendor.yaml manifests, pulling from Git/S3/HTTP/OCI/Terraform Registry. Use when vendoring Atmos components, configuring vendor.yaml manifests, or pulling modules from external sources."
metadata:
  version: "1.0.0"
---
...
- [Why Vendor](#why-vendor)
- [Types of Vendoring](#types-of-vendoring)
- [vendor.yaml Manifest Format](#vendoryaml-manifest-format)
...
- [Imports in Vendor Manifests](#imports-in-vendor-manifests)
- [Component Manifest (Legacy)](#component-manifest-legacy)
  - [Mixins (Legacy)](#mixins-legacy)
- [atmos vendor pull Command](#atmos-vendor-pull-command)
- [Version Pinning](#version-pinning)
- [Vendoring and Version Management Patterns](#vendoring-and-version-management-patterns)
  - [Single Version (Simplest)](#single-version-simplest)
  - [Folder-Based Versioning](#folder-based-versioning)
  - [Major.Minor Versioning](#majorminor-versioning)
...
Terraform root modules must exist locally; they cannot be pulled from remote sources at runtime the way child modules can. Vendoring makes this explicit: you copy the code once, commit it, and control when updates happen. You get real `git diff` visibility into code changes (not just version bumps), a full audit trail for compliance, the ability to patch vulnerabilities without waiting for upstream, IDE navigation across all code, and zero network dependencies during `terraform apply`.
...
Vendoring supports several strategies: single version (all environments use same version), folder-based versioning (multiple versions coexist via `metadata.component`), and major.minor grouping. For the full version management pattern catalog, see the `atmos-design-patterns` skill.
