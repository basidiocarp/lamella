---
name: atmos-stacks
description: "Stack configuration: imports, inheritance, deep merging, locals, vars, settings, metadata, overrides. Use when configuring Atmos stacks, setting up inheritance hierarchies, defining variables, or troubleshooting merge conflicts."
metadata:
  version: "1.0.0"
---
...
- [What Stacks Are](#what-stacks-are)
- [Stack Discovery](#stack-discovery)
  - [Stack Name Precedence](#stack-name-precedence)
- [Stack Manifest Structure](#stack-manifest-structure)
- [Configuration Sections Reference](#configuration-sections-reference)
  ...
  - [auth](#auth)
  - [overrides](#overrides)
- [Deep-Merge Behavior and Override Precedence](#deep-merge-behavior-and-override-precedence)
- [The _defaults.yaml Pattern](#the-_defaultsyaml-pattern)
- [Describing Stacks](#describing-stacks)
...
A common convention is to place `_defaults.yaml` files at each level of the directory hierarchy (org, tenant, stage). The underscore prefix ensures they sort first and they are excluded from stack discovery via `excluded_paths`. Each level imports its parent's defaults and adds its own.

For the full `_defaults.yaml` pattern with examples and directory structure, see the `atmos-design-patterns` skill.
...
Atmos provides YAML functions (`!terraform.output`, `!terraform.state`, `!store`, `!env`, `!exec`, `!include`) for dynamic value resolution at runtime. For the full reference, see the `atmos-yaml-functions` skill.
