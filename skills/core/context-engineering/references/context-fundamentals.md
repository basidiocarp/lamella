...
- Processing cost grows disproportionately with context length--not just double the cost for double the tokens, but exponentially more in time and computing resources
- Model performance degrades beyond certain context lengths even when the window technically supports more tokens
- Long inputs remain expensive even with prefix caching
