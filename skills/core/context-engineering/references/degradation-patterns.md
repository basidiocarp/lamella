...
Poisoning typically enters through three pathways:
1. Tool outputs may contain errors or unexpected formats that models accept as ground truth
2. Retrieved documents may contain incorrect or outdated information that models incorporate into reasoning
3. Model-generated summaries or intermediate outputs may introduce hallucinations that persist in context
...
- Tool misalignment where agents call wrong tools or parameters
- Hallucinations that persist despite correction attempts

Recovery requires removing or replacing poisoned content:
- Truncating context to before the poisoning point
- Explicitly noting the poisoning in context and asking for re-evaluation
- Restarting with clean context and preserving only verified information
...
Mitigate distraction through careful curation of what enters context:
- Apply relevance filtering before loading retrieved documents
- Use namespacing and organization to make irrelevant sections easy to ignore structurally
- Consider whether information truly needs to be in context or can be accessed through tool calls instead
...
Watch for:
- Responses that address the wrong aspect of a query
- Tool calls that seem appropriate for a different task
- Outputs that mix requirements from multiple sources

### Architectural Solutions

- Explicit task segmentation where different tasks get different context windows
...
Clash commonly arises from:
- Multi-source retrieval where different sources have contradictory information
- Version conflicts where outdated and current information both appear in context
- Perspective conflicts where different viewpoints are valid but incompatible

### Resolution Approaches

- Explicit conflict marking that identifies contradictions and requests clarification
- Priority rules that establish which source takes precedence
- Version filtering that excludes outdated information from context
...
Implement these strategies through specific architectural patterns:
- Use just-in-time context loading to retrieve information only when needed
- Use observation masking to replace verbose tool outputs with compact references
- Use sub-agent architectures to isolate context for different tasks
- Use compaction to summarize growing context before it exceeds limits

## Detecting Degradation in Prompt Design
...
## Mitigating Lost-in-Middle in Prompt Structure
